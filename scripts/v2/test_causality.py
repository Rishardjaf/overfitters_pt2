"""Causality test for the v2 feature panel.

Rebuild the panel on data truncated at a cut-off timestamp and compare every
feature for rows at or before the cut-off with the full build. A feature that
changes when later rows are added has read the future. This catches anything
the token-based audit cannot (a wrongly-signed merge, a centred window hidden in
a helper, a groupby that spans time).

    uv run python scripts/v2/test_causality.py
"""
...
def main() -> int:
    ...
    feats = sorted({c for cols in groups.values() for c in cols})
    ...
    if bad:
        ...
        return 1
    print(
        f"Causality test passed: {len(feats)} features identical on {len(a):,} rows before {CUTOFF}."
    )
    return 0
